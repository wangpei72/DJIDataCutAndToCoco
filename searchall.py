...
for index in range(4):
	for file in img_dirs[index]:
		...
		# 针对一对图片以及xml文件 判断图片与object是否有交集 坐上标记
		# 并且对独处的所有object做无重复无遗漏有效计数
		for i in range(3):
			for j in range(6):
				...
				for object in root_annotation.iter('object'):
					...
					if (xmin > xmax or ymin > ymax):
						...
						if object.find('name').text == 'armor':
							...
							img_file_list_armor.append(origin_img_file_name + '_' + str(i) + str(j) + '.jpg')
						else:
							# 针对装甲板之外的类别
							...
							img_file_list_other.append(origin_img_file_name + '_' + str(i) + str(j) + '.jpg')
		...
		for i in range(3):
			for j in range(6):
				# 再来一次遍历 这次往做了标记的碎片 保存 可视化（可选） 写属性xml
				# 写属性json
				if flag_cross[i][j]:
					image_id += 1
...
# print("id_armor", id_list_armor)
# print("id_other", id_list_other)
...
with open(out_anno_path+"test.json", "w+", encoding='utf-8') as f1:
	json.dump(json_dict_test, f1, indent=2, sort_keys=False, ensure_ascii=False)

with open(out_anno_path+"train.json", "w", encoding='utf-8') as f2:
	json.dump(json_dict_train, f2, indent=2, sort_keys=False, ensure_ascii=False)

with open(out_anno_path+"val.json", "w+", encoding='utf-8') as f3:
