...
out_img_path = "DJIDATACUT/newcut_test/image"
out_anno_path = "DJIDATACUT/newcut_test/image_annotation/"
    ...
    for file in img_dirs[index]:
        ...
        for object in root_annotation.iter('object'):
            if object.find('name').text == 'car':
                ...
                region_pil_arr = array(region_pil)
                ...
                file_name = ET.SubElement(new_root, 'filename')
                file_name.text = file_str_img
                size = ET.SubElement(new_root, 'size')
                width_xml = ET.SubElement(size, 'width')
                width_xml.text = str(region_pil_arr.shape[1])
                height_xml = ET.SubElement(size, 'height')
                height_xml.text = str(region_pil_arr.shape[0])
                object_xml =ET.SubElement(new_root, 'object')
                ob_name_xml = ET.SubElement(object_xml, 'name')
                ob_name_xml.text = other_name_str
                ob_bndbox_xml = ET.SubElement(object_xml, 'bndbox')
                bnd_x1 = ET.SubElement(ob_bndbox_xml, 'xmin')
                bnd_x1.text = str(true_xmin)
                bnd_y1 = ET.SubElement(ob_bndbox_xml, 'ymin')
                bnd_y1.text = str(true_ymin)
                bnd_x2 = ET.SubElement(ob_bndbox_xml, 'xmax')
                bnd_x2.text = str(true_xmax)
                bnd_y2 = ET.SubElement(ob_bndbox_xml, 'ymax')
                bnd_y2.text = str(true_ymax)
                bnd_w = ET.SubElement(ob_bndbox_xml, 'width')
                bnd_w.text = str("%.1f" % width)
                bnd_h = ET.SubElement(ob_bndbox_xml, 'height')
                bnd_h.text = str("%.1f" % height)
                for object in root_annotation.iter("object"):
                    if object.find('name').text == 'armor':
                        ...
                        if (xmin > xmax or ymin > ymax):
                            ...
                            if(A_ymax > B_ymax):
                                print("bndbox out of range,repairing...")
                                A_ymax = B_ymax
                            width_a = A_xmax - A_xmin
                            height_a = A_ymax - A_ymin
                            # true_width = 1.2 * width
                            # true_height = 1.2 * height
                            true_xmin_a = float("%.3f" % (0.1 * width + A_xmin - B_xmin))
                            true_xmax_a = float("%.3f" % (1.1 * width + A_xmax - B_xmax))
                            true_ymin_a = float("%.3f" % (0.1 * height + A_ymin - B_ymin))
                            true_ymax_a = float("%.3f" % (1.1 * height + A_ymax -B_ymax))
